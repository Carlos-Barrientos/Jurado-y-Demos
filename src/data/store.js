...
export function compressImageFile(file, maxWidth = 1600, maxHeight = 1600, quality = 0.82) {
  return new Promise((resolve) => {
    ...
    // If PDF or document, read as DataURL directly
...
export function saveState() {
  try {
    ...
    console.warn('LocalStorage quota reached, saving lightweight state without heavy inline binary data:', err);
    try {
      const lightweightDemos = (state.demos || []).map(d => ({
        ...d,
        infographicUrl: (d.infographicUrl && d.infographicUrl.length > 500000) ? '' : d.infographicUrl,
        images: (d.images || []).map(img => ({
          ...img,
          url: (img.url && img.url.length > 500000) ? '' : img.url
        }))
      }));
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        ...
        selectedUnit: state.selectedUnit,
        users: state.users,
        demos: lightweightDemos,
        posts: state.posts,
        deletedDemoIds: state.deletedDemoIds || [],
        deletedUserIds: state.deletedUserIds || []
      }));
    } catch (innerErr) {
      console.warn('LocalStorage save skipped (persisted in Firestore Cloud DB).');
    }
  }
}
